...
def check_for_log_entry(port='/dev/ttyCH9344USB0', baudrate=115200, search_string="CheckAndSetPca2131Bits", timeout=40):
    """
    监视串口日志，检查特定字符串是否出现。
    ...
    - port: str, 串口设备的路径。
    - baudrate: int, 串口通信的波特率。
    - search_string: str, 要搜索的日志条目。
    - timeout: int, 监控时间长度（秒）。
            ...
            while time.time() - start_time < timeout:
                if ser.inWaiting() > 0:
                    ...
                    if search_string in line:
                        print(f"Found '{search_string}' in the log.")
                        found = True
                        break
                    elif line:  # 打印所有接收到的有效日志
                        print(line)
        else:
            print("串口无法打开。")
    except Exception as e:
