...
public class combinationSum {
    ...
    public static void main(String[] args) {
        ...
        List<List<Integer>> list = new combinationSum().combinationSum(nums, 35);
        // list = clean(list);
    ...
    public void combine(int[] candidates, int target, List<Integer> list, int pos){
        if(target == 0){
        ...
        for(int i = pos; i < candidates.length; i++){
            if(candidates[i] <= target){
                ...
                combine(candidates, target-candidates[i], list, i);
